...
${g}`;console.log("[API] Contract text length:",m.length),console.log("[API] Analysis prompt length:",d.length),console.log("[API] Protocol prompt length:",h.length),console.log("[API] Protocol prompt first 200 chars:",h.slice(0,200));let[f,P]=await Promise.all([s.messages.create({model:"claude-sonnet-4-6",max_tokens:1e4,messages:[{role:"user",content:[...c,{type:"text",text:d}]}]}),s.messages.create({model:"claude-sonnet-4-6",max_tokens:8e3,messages:[{role:"user",content:[...c,{type:"text",text:h}]}]})]),y=extractText(f),w=extractText(P)||"{}";if(console.log("[API] Analysis stop_reason:",f.stop_reason,"| length:",y.length),console.log("[API] Protocol stop_reason:",P.stop_reason,"| length:",w.length),w.trim()&&"{}"!==w?console.log("[API] Protocol raw first 200 chars:",w.slice(0,200)):console.log("[API] PROTOCOL RESPONSE IS EMPTY"),!y.trim())throw Error("Анализ вернул пустой текст. Проверьте API ключ и содержимое файлов.");let A=function(e){let t=e.replace(/^```(?:json)?\s*/im,"").replace(/```\s*$/m,"").trim(),r=t.indexOf("{"),o=t.lastIndexOf("}");if(-1===r||-1===o)return{};try{return JSON.parse(t.slice(r,o+1))}catch{try{return JSON.parse(t.slice(r,o+1).replace(/[\x00-\x1F\x7F]/g," "))}catch(t){return console.error("Protocol JSON parse failed:",t,"\nRaw:",e.slice(0,400)),{}}}}(w);return console.log("[API] Protocol items count:",(A.protocolItems||[]).length),0===(A.protocolItems||[]).length&&console.warn("[API] ⚠️ protocolItems empty! Raw:",w.slice(0,600)),n.Z.json({contractTitle:A.contractTitle||r[0]?.name||"Договор",overallRisk:A.overallRisk||"medium",role:o,analysisDate:new Date().toLocaleDateString("ru-RU",{year:"numeric",month:"long",day:"numeric"}),analysisText:y,protocolItems:A.protocolItems||[]})}catch(t){console.error("Analysis error:",t);let e=t instanceof Error?t.message:"Неизвестная ошибка";return n.Z.json({error:`Ошибка при анализе договора: ${e}`},{status:500})}}let c=new a.AppRouteRouteModule({definition:{kind:s.x.APP_ROUTE,page:"/api/analyze/route",pathname:"/api/analyze",filename:"route",bundlePath:"app/api/analyze/route"},resolvedPagePath:"/Users/egorkolobov/game/contract-analyzer/app/api/analyze/route.ts",nextConfigOutput:"",userland:o}),{requestAsyncStorage:p,staticGenerationAsyncStorage:u,serverHooks:m,headerHooks:x,staticGenerationBailout:g}=c,d="/api/analyze/route"}};var t=require("../../../webpack-runtime.js");t.C(e);var __webpack_exec__=e=>t(t.s=e),r=t.X(0,[955,134],()=>__webpack_exec__(2823));module.exports=r})();