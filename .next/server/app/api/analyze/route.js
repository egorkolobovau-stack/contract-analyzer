...
${g}`,h=`Ты профессиональный российский юрист. Изучи договор и составь Протокол разногласий с позиции ${u}.
${x}
Найди все пункты договора которые ущемляют интересы ${u} и для каждого предложи альтернативную редакцию.
  ...
  "protocolItems": [
    {
      ...
      "proposedText": "Готовая альтернативная редакция — юридически грамотная, защищающая интересы ${u}",
      "justification": "Обоснование: почему оригинал ущемляет ${u} и чем обоснована правка",
...
${g}`;console.log("[API] Contract text length:",m.length),console.log("[API] Analysis prompt length:",d.length),console.log("[API] Protocol prompt length:",h.length);let[f,y]=await Promise.all([n.messages.create({model:"claude-sonnet-4-6",max_tokens:1e4,messages:[{role:"user",content:[...c,{type:"text",text:d}]}]}),n.messages.create({model:"claude-sonnet-4-6",max_tokens:8e3,messages:[{role:"user",content:[...c,{type:"text",text:h}]}]})]),P=extractText(f),w=extractText(y)||"{}";if(console.log("[API] Analysis stop_reason:",f.stop_reason,"| length:",P.length),console.log("[API] Protocol stop_reason:",y.stop_reason,"| length:",w.length),console.log("[API] Protocol raw preview:",w.slice(0,300)),!P.trim())throw Error("Анализ вернул пустой текст. Проверьте API ключ и содержимое файлов.");let _=function(e){let t=e.replace(/^```(?:json)?\s*/im,"").replace(/```\s*$/m,"").trim(),r=t.indexOf("{"),o=t.lastIndexOf("}");if(-1===r||-1===o)return{};try{return JSON.parse(t.slice(r,o+1))}catch{try{return JSON.parse(t.slice(r,o+1).replace(/[\x00-\x1F\x7F]/g," "))}catch(t){return console.error("Protocol JSON parse failed:",t,"\nRaw:",e.slice(0,400)),{}}}}(w);return console.log("[API] Protocol items count:",(_.protocolItems||[]).length),0===(_.protocolItems||[]).length&&console.warn("[API] ⚠️ protocolItems empty! Raw:",w.slice(0,600)),s.Z.json({contractTitle:_.contractTitle||r[0]?.name||"Договор",overallRisk:_.overallRisk||"medium",role:o,analysisDate:new Date().toLocaleDateString("ru-RU",{year:"numeric",month:"long",day:"numeric"}),analysisText:P,protocolItems:_.protocolItems||[]})}catch(t){console.error("Analysis error:",t);let e=t instanceof Error?t.message:"Неизвестная ошибка";return s.Z.json({error:`Ошибка при анализе договора: ${e}`},{status:500})}}let c=new a.AppRouteRouteModule({definition:{kind:n.x.APP_ROUTE,page:"/api/analyze/route",pathname:"/api/analyze",filename:"route",bundlePath:"app/api/analyze/route"},resolvedPagePath:"/Users/egorkolobov/game/contract-analyzer/app/api/analyze/route.ts",nextConfigOutput:"",userland:o}),{requestAsyncStorage:p,staticGenerationAsyncStorage:u,serverHooks:m,headerHooks:x,staticGenerationBailout:g}=c,d="/api/analyze/route"}};var t=require("../../../webpack-runtime.js");t.C(e);var __webpack_exec__=e=>t(t.s=e),r=t.X(0,[955,134],()=>__webpack_exec__(2823));module.exports=r})();